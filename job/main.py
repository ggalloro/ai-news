...
def summarize_entries(entries, api_key):
    ...
    for entry in entries:
        ...
        prompt = f"""
        Il tuo ruolo è quello di un conduttore di podcast professionale che scrive uno script per un briefing audio in italiano sull'Intelligenza Artificiale.
        Il tuo compito è riassumere il seguente articolo.
        
        Linee guida per il riassunto:
        - Scrivi in uno stile podcast naturale, conversazionale e coinvolgente.
        - L'output deve essere un paragrafo pulito di testo semplice in italiano.
        - Deve essere adatto per la conversione diretta text-to-speech.

        **ISTRUZIONI CRITICHE:**
        - **NON** usare alcuna formattazione Markdown (es. niente asterischi, niente cancelletti, niente liste).
        - **NON** iniziare con riempitivi conversazionali come "Naturalmente, ecco un riassunto..." o "Ecco un riassunto...".
        - **NON** annunciare quello che stai facendo. Fornisci direttamente il riassunto.
        - Scrivi completamente in italiano, traducendo tutti i contenuti dall'inglese.
...
def generate_and_upload_stitched_audio(summaries, bucket_name):
    if not summaries:
        print("Nessun riassunto per cui generare audio.")
        return None
    try:
        tts_client = texttospeech.TextToSpeechClient()
        storage_client = storage.Client()
        voice = texttospeech.VoiceSelectionParams(language_code="it-IT", name="it-IT-Chirp3-HD-Autonoe")
        audio_config = texttospeech.AudioConfig(audio_encoding=texttospeech.AudioEncoding.MP3)
        temp_dir = "/tmp/temp_audio"
        os.makedirs(temp_dir, exist_ok=True)
        all_audio_segments = []
        intro_text = "Buongiorno e benvenuti al vostro briefing IA. Ecco le ultime notizie."
        ...
        for summary in summaries:
            title_text = f"La prossima storia è intitolata: {summary['title']}."
        ...
        if not all_audio_segments:
            print("Nessun segmento audio è stato generato.")
            return None
...
def main():
    ...
    print("Avvio del job di generazione audio RSS...")
    try:
        storage_client = storage.Client()
        ...
        if blob.exists():
            try:
                ...
                print(f"Impossibile analizzare il file dei tempi dell'ultima elaborazione, ripartenza da zero. Errore: {e}")
        ...
        if new_entries:
            ...
            if public_url:
                print(f"Audio generato e caricato con successo: {public_url}")
                blob.upload_from_string(json.dumps(latest_times, indent=4))
                print("Job completato con successo.")
                sys.exit(0)
            else:
                print("Job fallito durante la generazione audio.")
                sys.exit(1)
        else:
            print("Nessuna nuova voce trovata. Job completato con successo.")
            sys.exit(0)
